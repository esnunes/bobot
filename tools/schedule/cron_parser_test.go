...
func TestMinInterval(t *testing.T) {
	tests := []struct {
		...
		{"every hour", "0 * * * *", 1 * time.Hour},
		{"daily", "0 9 * * *", 24 * time.Hour},
		{"weekdays", "0 9 * * 1-5", 24 * time.Hour},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := MinInterval(expr)
			if got != tt.expected {
				t.Errorf("MinInterval(%q) = %v, want %v", tt.expr, got, tt.expected)
			}
		})
	}
}

func TestMinInterval_TooFrequent(t *testing.T) {
	// Every minute should be below 15-minute threshold
	expr, _ := Parse("* * * * *")
	interval := MinInterval(expr)
	if interval >= 15*time.Minute {
		t.Errorf("expected interval < 15m, got %v", interval)
	}

	// Every 15 minutes should be at threshold
	expr, _ = Parse("*/15 * * * *")
	interval = MinInterval(expr)
	if interval < 15*time.Minute {
		t.Errorf("expected interval >= 15m, got %v", interval)
